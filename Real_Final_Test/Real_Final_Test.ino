...
float ddongz, humiz, tempz =0; //측정용 똥 휴미 템프 값
...
unsigned long pt4 = 0;
unsigned long pt5 = 0;
unsigned long pt6 = 0; //30초마다 값 보낼 때 사용하기 위한 pt
unsigned long ct = 0;
unsigned long ct2 = 0;
unsigned long ct3 = 0;
unsigned long ct4 = 0;
unsigned long ct5 = 0;
unsigned long ct6 = 0; //30초마다 값 보낼 때 사용하기 위한 ct
...
void loop() {
  ...
  ct5=millis();
  ct6=millis();
  ddongz=analogRead(gassensor);
  humiz=dht.readHumidity();
  tempz=dht.readTemperature();
...
if (count >= 20) {
 ...
 if((ct6-pt6) >= 30000){
  Serial.println("테스트용 30초간격 측정 일반 루프에서 값입니다.");

      Serial.print("ddongz =   ");    
      Serial.println(ddongz);
      Serial.print("humiz =   ");
      Serial.println(humiz);
      Serial.print("tempz =   ");
 ...
 if ( ((ct3 - pt3) > 240000)){
      ...
      Serial.print("temp / temp_average :   ");
      Serial.println((temp / temp_average));
      Serial.print("humi / humi_average :   ");
      Serial.println((humi / humi_average));
      Serial.print("ddong / ddong_average :   ");
